...
void PaymentServiceJsonRpcServer::processJsonRpcRequest(const Common::JsonValue& req, Common::JsonValue& resp) {
  try {
    ...
    if (!config.legacySecurity) {
      ...
      if (clientPassword != config.m_rpcPassword) {
        makeInvalidPasswordResponse(resp);
        return;
      }
    }
...
std::error_code PaymentServiceJsonRpcServer::handleChangePassword(const ChangePassword::Request& request, ChangePassword::Response& response) {
  return service.changePassword(request.oldPassword, request.newPassword, response.status);
}
...
std::error_code PaymentServiceJsonRpcServer::handleGetTransactionCount(const GetTransactionCount::Request& request, GetTransactionCount::Response& response) {
  if (!request.blockHash.empty()) {
...
std::error_code PaymentServiceJsonRpcServer::handleGetViewKeys(const GetViewKeys::Request& request, GetViewKeys::Response& response) {
  return service.getViewKeys(response.viewPublicKey, response.viewSecretKey);
}
