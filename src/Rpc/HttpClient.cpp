...
void HttpClient::connect() {
  std::string hostname;
  getHostName(m_address, hostname);
  if (m_ssl_enable) {
    try {
      ...
      m_ssl_sock.reset(new boost::asio::ssl::stream<tcp::socket> (m_io_service, std::ref(ctx)));
      tcp::resolver resolver(m_io_service);
      tcp::resolver::query query(hostname, std::to_string(m_port));
      boost::asio::connect(m_ssl_sock->lowest_layer(), resolver.resolve(query));
#if defined(_WIN32)
      sockSetup((SOCKET &) m_ssl_sock->lowest_layer().native_handle());
#endif
      m_ssl_sock->lowest_layer().set_option(tcp::no_delay(true));
      m_ssl_sock->lowest_layer().set_option(boost::asio::socket_base::keep_alive(true));
      if (!m_ssl_no_verify) {
        m_ssl_sock->set_verify_mode(boost::asio::ssl::verify_peer);
        SSL_set_tlsext_host_name(m_ssl_sock->native_handle(), hostname.c_str());
      } else {
        m_ssl_sock->set_verify_mode(boost::asio::ssl::verify_none);
      }
      m_ssl_sock->set_verify_callback(boost::asio::ssl::rfc2818_verification(hostname));
      m_ssl_sock->handshake(boost::asio::ssl::stream_base::client);
      m_connected = true;
