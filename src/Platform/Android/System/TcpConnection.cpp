    ...
    if (dispatcher != nullptr) {
        ...
        if (close(connection) == -1) {
            throw std::runtime_error(
                "TcpConnection::operator=, close failed, " + lastErrorMessage()
            );
        }
    }
        ...
        if (errno != EAGAIN) {
            ...
            if (epoll_ctl(dispatcher->getEpoll(), EPOLL_CTL_MOD, connection, &connectionEvent)==-1){
                ...
                dispatcher->getCurrentContext()->interruptProcedure = [&]() {
                    ...
                    if (epoll_ctl(dispatcher->getEpoll(),
                                  EPOLL_CTL_MOD,
                                  connection,
                                  &connectionEvent) == -1) {
                        throw std::runtime_error(
                            "TcpConnection::stop, epoll_ctl failed, " + lastErrorMessage()
                        );
                    }
...
std::size_t TcpConnection::write(const uint8_t *data, size_t size)
{
    ...
    if(size == 0) {
        if(shutdown(connection, SHUT_WR) == -1) {
            throw std::runtime_error(
                "TcpConnection::write, shutdown failed, " + lastErrorMessage()
            );
        }
    ...
    if (transferred == -1) {
        if (errno != EAGAIN) {
            ...
            if (epoll_ctl(dispatcher->getEpoll(), EPOLL_CTL_MOD, connection, &connectionEvent)==-1){
                ...
                if((operationContext.events & (EPOLLERR | EPOLLHUP)) != 0) {
                    throw std::runtime_error(
                        "TcpConnection::write, events & (EPOLLERR | EPOLLHUP) != 0"
                    );
                }
...
std::pair<Ipv4Address, uint16_t> TcpConnection::getPeerAddressAndPort() const
{
    ...
    if (getpeername(connection, reinterpret_cast<sockaddr*>(&addr), &size) != 0) {
        throw std::runtime_error(
            "TcpConnection::getPeerAddress, getpeername failed, " + lastErrorMessage()
        );
    }
...
TcpConnection::TcpConnection(Dispatcher &dispatcher, int socket)
    : dispatcher(&dispatcher), connection(socket)
{
    contextPair.readContext = nullptr;
    contextPair.writeContext = nullptr;
    epoll_event connectionEvent;
    ...
    if (epoll_ctl(dispatcher.getEpoll(), EPOLL_CTL_ADD, socket, &connectionEvent) == -1) {
        throw std::runtime_error(
            "TcpConnection::TcpConnection, epoll_ctl failed, " + lastErrorMessage()
        );
    }
}
